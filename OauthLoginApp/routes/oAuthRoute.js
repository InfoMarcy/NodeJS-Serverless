...
const validateUser = require('../model/oAuth2/oAuthUserModel');
...
router.post("/usuario/register", (req, res, next) => {
  ...
  OAuthUsersModel.findOne({ email: req.body.email }, function(err, user) {
    if (user) {
      res.status(400).send({ success: false, mensaje: "El correo ya se encuentra registrado" });
    }
  });
  ...
  OAuthUsersModel.register(nuevoUsuario, function(err, user) {
    ...
    res.send(user);
    res.status(200).send({ success:true, mensaje:'Usuario registrado exitosamente'})
  });
});
